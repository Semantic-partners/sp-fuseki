...
- **Evidence:** migrating a `chinook` test rig (4 datasets, Lucene text index)
  off `stain/jena-fuseki:5.1.0`, and booting an Elasticsearch-plugin
...
- `text:query` returns hits on 41,174 real triples; both entity-map fields resolve
  independently;
- a real integration's production config boots unchanged — legacy `fuseki:serviceQuery` syntax
